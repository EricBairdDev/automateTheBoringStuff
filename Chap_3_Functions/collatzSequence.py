def collatz(number):
    if number % 2 == 0: # If number is even //2, if odd 3 * number + 1
        collNumber = number // 2
        return collNumber
    else:
        collNumber = 3 * number + 1
        return collNumber

print('Enter a whole number greater than 0:') # Prompt user
while True:
    try:
        number = int(input())
        if number <= 0:   # Make sure it is a positive integer and larger than 0
            print('Please enter a whole number greater than 0:')
            continue
        else:
            break
    except ValueError:  # Exception for entering a non integer
        print('Please enter an integer:')
        continue
while number != 1: # Ends sequence at 1
...
# and return this value. If number is odd, then collatz() should print
...
# integer — sooner or later, using this sequence, you’ll arrive at 1!
# Even mathematicians aren’t sure why. Your program is exploring
# what’s called the Collatz sequence, sometimes called “the simplest
